...
def calculate_single_MD_nRE(y, y_hat, n, p):
    return ((math.pow(y, 1/n) - math.pow(y_hat, 1/n)))**p

# Check the input
def checkInput(y, y_hat, n, p):
    try:
        float(y)
        float(y_hat)
        int(n)
...
def main():
    ...
    y, y_hat, n, p = checkInput(y, y_hat, n, p)
    print("The value of single MD-nRE is:", calculate_single_MD_nRE(y, y_hat, n, p))
